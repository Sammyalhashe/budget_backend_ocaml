...
let exchange_public_token public_token =
  ...
  >>= fun (_json, item_id, access_token) ->
  Db.save_token item_id access_token None >>= fun () ->
  Lwt.return
    (`Assoc
       [ ("access_token", `String access_token); ("item_id", `String item_id) ])
...
let handle_webhook payload =
  ...
  in
  let item_id =
    match List.assoc_opt "item_id" fields with Some (`String s) -> s | _ -> ""
  in
  let error =
    match List.assoc_opt "error" fields with
    | Some (`Assoc err_fields) ->
      let error_message =
        match List.assoc_opt "error_message" err_fields with
        | Some (`String s) -> s
        | _ -> ""
      in
      Some error_message
  ...
  in
  Plaid_notifier.notify event >>= fun () -> Lwt.return_unit
