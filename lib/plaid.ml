...
let client_id =
  Sys.getenv_opt "PLAID_CLIENT_ID" |> Option.value ~default:"dummy_client_id"

let secret =
  Sys.getenv_opt "PLAID_SECRET" |> Option.value ~default:"dummy_secret"
...
let create_link_token ?(hosted_link = false) ?webhook () =
  ...
  let fields_with_hosted_link =
    if hosted_link
    then ("hosted_link", `Assoc []) :: fields_with_webhook
    else fields_with_webhook
  in
  ...
  >>= fun (_resp, body) -> Cohttp_lwt.Body.to_string body >|= from_string
...
let fire_test_webhook access_token =
  let uri = Uri.of_string (base_url ^ "/sandbox/item/fire_webhook") in
  let body_json =
    `Assoc
      [
        ("client_id", `String client_id);
