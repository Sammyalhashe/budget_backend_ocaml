...
let () =
  ...
  Dream.run ~port:5000 @@ Dream.logger
  @@ Dream.router
       [
...
|}
         ...
         Dream.post "/api/plaid/create_link_token" (fun _ ->
           Plaid_handler.create_link_token () >>= fun json ->
         ...
         Dream.post "/api/plaid/get_transactions" (fun request ->
           Dream.body request >>= fun body_str ->
           let payload =
             try Yojson.Safe.from_string body_str with _ -> `Assoc []
           in
           let open Yojson.Safe.Util in
           let access_token =
             payload |> member "access_token" |> to_string_option
           in
           let start_date =
             payload |> member "start_date" |> to_string_option
             |> Option.value ~default:(get_iso_date 730)
             (* 2 years ago *)
           in
           let end_date =
             payload |> member "end_date" |> to_string_option
             |> Option.value ~default:(get_iso_date 0)
             (* today *)
           in
               ...
               (fun exn ->
                 ...
                 let is_reauth_required =
                   try
                     let _ =
                       Str.search_forward
                         (Str.regexp "ITEM_LOGIN_REQUIRED")
                         err_msg 0
                     in
                     true
           ...
           | None -> Dream.respond ~status:`Bad_Request "Missing access_token");
             ...
             (Yojson.Safe.to_string
                (`Assoc
                   [
                     ...
                     ("message", `String "Deleted errored tokens");
                   ])));
         Dream.get "/api/plaid/ws" (fun _req ->
           Dream.websocket (fun websocket ->
             Plaid_notifier.add_subscriber (fun event ->
               ...
               Lwt.catch
                 (fun () ->
                   Dream.send websocket json >>= fun () -> Lwt.return_unit)
         ...
         Dream.post "/api/plaid/start-auth" (fun _req ->
           ...
           let hosted_link_url =
             ...
             | _ -> ""
           in
           Db.save_link_session link_token link_token hosted_link_url "pending"
           >>= fun () ->
             ...
             let response =
               `Assoc
                 [
                   ...
                   ( "item_id",
                     match item_id with Some id -> `String id | None -> `Null );
                   ( "link_token",
                     match link_token with Some t -> `String t | None -> `Null
                   );
         ...
         Dream.post "/api/plaid/fire_test_webhook" (fun req ->
           ...
           | _ -> (
             let open Yojson.Safe.Util in
             match payload |> member "access_token" |> to_string_option with
             | None -> Dream.respond ~status:`Bad_Request "Missing access_token"
             | Some access_token ->
               let%lwt json = Plaid.fire_test_webhook access_token in
               Dream.json (Yojson.Safe.to_string json)));
         ...
         Dream.get "/api/plaid/wait-auth" (fun _req ->
           ...
           let rec poll () =
             ...
             | Some (item_id, access_token, link_token, status, _) -> (
               let has_id =
                 match item_id with Some id -> id <> "" | None -> false
               in
               let has_at =
                 match access_token with Some at -> at <> "" | None -> false
               in
               Dream.info (fun m ->
                 m "wait-auth: status=%s, has_item_id=%b, has_access_token=%b"
                   status has_id has_at);
               if status = "connected" && has_at
               then Dream.json (Yojson.Safe.to_string success_response)
               else
                 match link_token with
                 | Some lt -> (
                   let current_time = Unix.gettimeofday () in
                   if current_time -. start_time > timeout
                   then Dream.respond ~status:`Request_Timeout "Auth timeout"
                   else
                       ...
                       try
                         ...
                         List.find_map
                           (fun s ->
                             match
                               s |> member "results"
                               |> member "item_add_results" |> to_list
                             with
                             | res :: _ -> Some res
                             | [] -> None)
                           sessions
                     ...
                     | Some res ->
                       let public_token =
                         res |> member "public_token" |> to_string
                       in
                       Dream.info (fun m ->
                         m "wait-auth: Success! Exchanging token...");
                       Plaid.exchange_public_token public_token
                       >>= fun (_exchange_json, item_id, access_token) ->
                       Dream.info (fun m ->
                         m "SAVE_TOKEN item_id: %s, access_token: %s" item_id
                           access_token);
                       Db.save_token item_id access_token (Some lt)
                       >>= fun () ->
                       Db.update_link_session_status lt "connected"
                       >>= fun () ->
                       Dream.json (Yojson.Safe.to_string success_response)
                     | None -> Lwt_unix.sleep 2.0 >>= fun () -> poll ())
                 | None -> Lwt_unix.sleep 2.0 >>= fun () -> poll ())
             | None ->
